...
export default function GalleryPage() {
  ...
  useEffect(() => {
    if (!hasSetDefault.current && departments && departments.length > 0 && filters.departmentId === null && filters.q === '') {
      hasSetDefault.current = true
      const random = departments[Math.floor(Math.random() * departments.length)]
      setFilters({ departmentId: random.id })
    }
  }, [departments, filters.departmentId, filters.q, setFilters])

  const { artworks, total, isLoadingSearch, isLoadingArtworks, error, hasMore, loadMore } =
    useArtworkSearch(filters)

  if (error) {
    return (
      <Container maxWidth="xl" sx={{ py: 3 }}>
        <Alert severity="error">Failed to load artworks. Please try again later.</Alert>
      </Container>
    )
  }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      <ArtworkGrid
        ...
        loading={isLoadingSearch || (artworks.length === 0 && isLoadingArtworks)}
